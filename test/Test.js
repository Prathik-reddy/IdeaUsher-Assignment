...
describe("Election", function () {
  ...
  describe('Checking whole election system', () => {
    ...
    it("check all voting related functions", async () => {
      ...
      const hasV2Voted = await voter.hasVoted(v2.address);
      const hasV3Voted = await voter.hasVoted(v3.address);

      expect(hasV1Voted).to.be.equal(true);
      expect(hasV2Voted).to.be.equal(true);
      expect(hasV3Voted).to.be.equal(true);

      const stop = await election.connect(organizer).stopElection();
      const isElecStop = await election.isElectionStopped();
      expect(isElecStop).to.be.equal(true);


      const winner = await election.connect(organizer).winner();
      expect(winner).to.be.equal(c1.address);
    })
  })
})
